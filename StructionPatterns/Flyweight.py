...
class Flyweight():
    ...
    def operation(self, unique_state: str) -> None:
        ...
        print(f"Flyweight: Отображение общего ({s}) и уникального ({u}) состояния.", end="")
...
class FlyweightFactory():
    ...
    def get_flyweight(self, shared_state: Dict) -> Flyweight:
        ...
        if not self._flyweights.get(key):
            print("FlyweightFactory: Не могу найти объект легковеса, создаю новый.")
            self._flyweights[key] = Flyweight(shared_state)
        else:
            print("FlyweightFactory: Повторное использование существующего объекта легковеса.")

        return self._flyweights[key]

    def list_flyweights(self) -> None:
        count = len(self._flyweights)
        print(f"FlyweightFactory: Я имею {count} объекта легковесов:")
...
def add_car_to_police_database(
    ...
    print("\n\nClient: Добавление автомобиля в базу данных.")
